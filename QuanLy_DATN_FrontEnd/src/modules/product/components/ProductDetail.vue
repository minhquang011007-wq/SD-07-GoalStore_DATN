  ...
  <div class="rounded-2xl border bg-white p-4 shadow-sm">
    ...
    <div v-else-if="selectedProduct" class="space-y-5 pt-4">
      <div class="flex gap-3">
        <img v-if="selectedProduct.thumbnailUrl" :src="resolveProductImageUrl(selectedProduct.thumbnailUrl)" class="h-20 w-20 shrink-0 rounded-2xl border bg-slate-50 object-cover" />
        <div>
      ...
      <div>
        ...
        <div class="space-y-2">
          <div v-for="history in historyItems" :key="history.id" class="rounded-xl border p-3 text-sm">
          ...
          <div v-if="historyItems.length === 0" class="rounded-xl border p-3 text-sm text-slate-500">Chưa có history.</div>
        </div>
      </div>

      <div class="flex flex-wrap gap-2">
        <button @click="emit('edit-product', selectedProduct.id)" class="rounded-lg border px-3 py-2 text-sm hover:bg-slate-50">Sửa sản phẩm</button>
        <button @click="emit('hide-oos', selectedProduct.id)" class="rounded-lg border px-3 py-2 text-sm hover:bg-slate-50">Hide if OOS</button>
        <button @click="emit('delete-soft', selectedProduct.id)" class="rounded-lg border px-3 py-2 text-sm text-rose-600 hover:bg-rose-50">Xóa mềm</button>
      </div>
    </div>
