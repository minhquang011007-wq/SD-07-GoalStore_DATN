  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
    <div class="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl">
      <div class="flex items-center justify-between">
        <h3 class="text-lg font-semibold">{{ editingCategoryId ? 'Sửa danh mục' : 'Thêm danh mục' }}</h3>
        <button @click="emit('close')" class="rounded-lg border p-2"><X :size="16" /></button>
      </div>
      <div class="mt-4 grid gap-4">
        <label class="space-y-2"><span class="text-sm">Tên danh mục</span><input v-model="form.name" class="w-full rounded-xl border px-3 py-2" /></label>
      ...
      <div class="mt-6 flex justify-end gap-3">
        ...
        <button @click="emit('submit')" :disabled="saving" class="rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{{ editingCategoryId ? 'Lưu thay đổi' : 'Tạo danh mục' }}</button>
      </div>
    </div>
  </div>
</template>
